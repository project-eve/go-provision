...
// baseOsMgr publishes these config/status files
...
func checkBaseOsStorageDownloadStatus(uuidStr string,
	...
	if ret.MinState < types.DOWNLOADED {
		log.Printf("checkBaseOsStorageDownloadStatus(%s) for %s, Download in-progress\n",
			config.BaseOsVersion, uuidStr)
		return ret.Changed, false
	}

	log.Printf("checkBaseOsStorageDownloadStatus(%s) for %s, Download done\n",
		config.BaseOsVersion, uuidStr)
...
func checkBaseOsVerificationStatus(uuidStr string,
	...
	if ret.MinState < types.DELIVERED {
		log.Printf("checkBaseOsVerificationStatus(%s) for %s, Verification in-progress\n",
			config.BaseOsVersion, uuidStr)
		return ret.Changed, false
	}
	log.Printf("checkBaseOsVerificationStatus(%s) for %s, Verification done\n",
		config.BaseOsVersion, uuidStr)
	return ret.Changed, true
}
	...
	if del {
		...
		if ok := baseOsStatusDelete(uuidStr); ok {
			statusFilename := fmt.Sprintf("%s/%s.json",
				baseOsMgrBaseOsStatusDirname, uuidStr)
...
func doBaseOsUninstall(uuidStr string, status *types.BaseOsStatus) (bool, bool) {
	...
	for i, _ := range status.StorageStatusList {
		...
		if ss.HasVerifierRef {
			...
			removeVerifierConfig(baseOsObj, ss.ImageSha256)
			ss.HasVerifierRef = false
			changed = true
		}

		_, err := lookupVerificationStatusSha256(baseOsObj, ss.ImageSha256)
	...
	for i, _ := range status.StorageStatusList {
		...
		_, err := lookupDownloaderStatus(baseOsObj, ss.ImageSha256)
		// XXX if additional refs it will not go away
		if false && err == nil {
...
func checkInstalledVersion(config types.BaseOsConfig) string {
	...
	log.Printf("%s, installed baseOs %s\n",
		config.PartitionLabel, config.BaseOsVersion)
	return ""
}
...
func writeBaseOsStatus(baseOsStatus *types.BaseOsStatus, uuidStr string) {
	...
	statusFilename := baseOsMgrBaseOsStatusDirname + "/" + uuidStr + ".json"
